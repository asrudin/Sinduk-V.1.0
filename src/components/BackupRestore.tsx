...
export default function BackupRestore({
  ...
  const handleRestoreSubmit = (e: React.FormEvent) => {
    ...
    if (ok) {
      setSuccess('Restorasi database berhasil! Seluruh data state Buku Induk telah dipulihkan.');
      setFileText('');
    } else {
      setError('Format data tidak valid! Pastikan berkas .json tersebut berasal dari ekspor cadangan Buku Induk Siswa Digital.');
    }
  };
  ...
  return (
    <div className="space-y-6 z-10 relative">
      ...
      <div>
        ...
        <p className="text-xs text-slate-400 mt-0.5 font-medium">Cadangkan database Buku Induk Anda ke file lokal atau pulihkan cadangan lama untuk pencegahan kehilangan data</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass-card p-6 rounded-2xl space-y-5 border border-white/10">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-1 leading-relaxed">Ekspor seluruh data siswa, profil lengkap, daftar kelas, tahun ajaran, catatan prestasi/pelanggaran, serta akun guru ke dalam satu berkas terenkripsi JSON.</p>
          </div>
          ...
          <div className="border-t border-white/5 pt-5 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <button
                ...
                onClick={() => {
                  if (confirm('PERINGATAN! Seluruh data perubahan Anda saat ini akan dihapus dan dikembalikan ke data simulasi default Buku Induk Sekolah. Lanjutkan?')) {
                    onResetToDefault();
                  }
                }}
                className="w-full flex items-center justify-center gap-2 px-3 py-2 border border-indigo-500/20 hover:bg-indigo-500/10 text-indigo-400 font-semibold text-xs rounded-xl transition duration-150 cursor-pointer"
              >
              ...
              <button
                id="btn-trigger-wipe"
                onClick={() => {
                  if (confirm('PERINGATAN KERAS! Tindakan ini akan menghapus seluruh data siswa, kelas, tahun akademik, catatan perkembangan, dan guru pendidik (kosong total). Tindakan ini tidak dapat dibatalkan! Apakah Anda yakin?')) {
                    onWipeAllData();
                  }
                }}
        ...
        <div className="glass-card p-6 rounded-2xl space-y-5 border border-white/10">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-1 leading-relaxed">Tarik & lepas file cadangan berformat <span className="font-mono bg-white/10 text-white px-1.5 py-0.5 rounded">.json</span> ke panel interaktif di bawah untuk memulihkan database Buku Induk.</p>
          </div>
          ...
          <form onSubmit={handleRestoreSubmit} className="space-y-4 pt-1">
            <div className="space-y-2">
              ...
              >
                <Upload className={`h-8 w-8 transition ${isDragOver ? 'text-indigo-400 animate-bounce' : 'text-slate-400'}`} />
